...
func (tc *ConsulE2ETest) TestConsulRegistration(f *framework.F) {
	...
	testutil.WaitForResult(func() (bool, error) {
		...
		for _, s := range services {
			sort.Strings(s.ServiceTags)

			// If we've made it this far the tags should *always* match
			require.Equal(t, expectedTags, s.ServiceTags)
		}
		return true, nil
	}, func(err error) {
...
func (tc *ConsulE2ETest) TestCanaryInplaceUpgrades(f *framework.F) {
	...
	testutil.WaitForResult(func() (bool, error) {
		...
		for _, s := range consulServices {
			if e2eutil.ConsulTagsEqual(s, []string{"canary", "foo"}) {
				found = true
				break
			}
		}
		return found, fmt.Errorf(`could not find service tags {"canary", "foo"}: %#v`, consulServices)
	}, func(err error) {
	...
	testutil.WaitForResult(func() (bool, error) {
		...
		for _, s := range consulServices {
			if !e2eutil.ConsulTagsEqual(s, expected) {
